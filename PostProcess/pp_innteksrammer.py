...
class ETL(IRData_postprocess):
    ...
    def calc_kostnadsgrunnlag(self):
        return self.calc_aarslonnjusterte_d_v_kostnader() + self.avskrivninger + self.nettapskostnad_ld + self.nettapskostnad_rd + self.calc_kpi_justert_kile()  + self.rd_cga + self.avkasntningsgrunnlag * self.calc_referanserente()
    
    def calc_inntektsramme_foer_kalibrering(self):
        kostnadsgrunnlag = self.calc_kostnadsgrunnlag().fillna(0)
        rho = self.config['forutsetninger']['rho']
        
        deanorm_rnett_object: DEAnormRnett = DEAnormRnett()
        deanorm_rnett_df: pd.DataFrame = deanorm_rnett_object.create_DEAnormRnett_DataFrame()
        deanorm_dnett_object: DEAnormDnett = DEAnormDnett()
        deanorm_dnett_df: pd.DataFrame = deanorm_dnett_object.create_DEAnormDnett_DataFrame()
        
        dea_dnett = deanorm_dnett_df['Kalibrert DEAnorm'].fillna(0)
        dea_rnett = deanorm_rnett_df['Kalibrert DEAnorm'].fillna(0)
        return (1 - rho) * kostnadsgrunnlag + (dea_dnett + dea_rnett) * rho
    ...
    def calc_k_star_etter_kalibrering(self):
        ...
        
    def calc_inntektsramme_etter_kalibrering(self):
        ...
    ...
    def create_ETL_DataFrame(self):
        ...
        sum_kostnader =  self.calc_aarslonnjusterte_d_v_kostnader() + self.avskrivninger + self.nettapskostnad_ld + self.nettapskostnad_rd + self.calc_kpi_justert_kile()  + self.rd_cga
        ...
        inntektsramme_foer_kalibrering = self.calc_inntektsramme_foer_kalibrering() 
        kostnadsnorm_lokalt_dnett = deanorm_dnett_df['Kalibrert DEAnorm'] #+ self.nettapskostnad_ld #nettapskostnad taes ikke med i kostnadsnormen (excelarket)
        kostnadsnorm_regionalt_dnett = deanorm_rnett_df['Kalibrert DEAnorm'] + self.nettapskostnad_rd # regionalnett tar med nettapskostnad i kostnadsnormen (excelarket)
        tillegg_i_kostnadsnorm_for_kundevekst = self.calc_tillegg_i_kostnadsnorm_for_kundevekst()
        kostnadsnorm_transmisjonsnett = 0
        
        
        sum_IR = np.nansum(inntektsramme_foer_kalibrering)
        sum_K = np.nansum(kostnadsgrunnlag)
        sum_IR_K = sum_IR - sum_K
        sum_avkastningsgrunnlag = np.nansum(self.avkasntningsgrunnlag)
        sum_IR_k_per_sum_AKG = sum_IR_K / sum_avkastningsgrunnlag
        sum_IR_k_per_sum_AKG: float = -0.023 / 100 #TODO: Fix this hack #NOTE in per cent
        print(f"Sum IR: {sum_IR}, Sum K: {sum_K}, Sum IR-K: {sum_IR_K}, Sum IR-K per sum AKG: {sum_IR_k_per_sum_AKG}", 'avkastningsgrunnlag: ', sum_avkastningsgrunnlag)
        print('------------------------')
        
        
        sum_kostnadsgrunnlag_u_ = None
        sum_faktisk_kostnad= None
        total_avvik= None
        renteeffekt_opå_avvik= None
        sum_renter_paa_avvik_per_sum_AKG: float = -0.098 / 100 #TODO: fix this hack #Note in per cent
        print(sum_renter_paa_avvik_per_sum_AKG) 
        k_etter_kalibrering = ((kostnadsnorm_lokalt_dnett +  kostnadsnorm_regionalt_dnett + kostnadsnorm_transmisjonsnett) - self.avkasntningsgrunnlag * (sum_IR_k_per_sum_AKG + sum_renter_paa_avvik_per_sum_AKG ) / self.config['forutsetninger']['rho']) + tillegg_i_kostnadsnorm_for_kundevekst

        print('------------------------')
        print(sum_IR_k_per_sum_AKG) 
        print('------------------------')
        ...
        data: pd.DataFrame = {
            ...
            'Selskap': self.comp,
            'rho': self.config['forutsetninger']['rho'],
            'sum ir k per sum akg': sum_IR_k_per_sum_AKG,
            'sum renter avvik': sum_renter_paa_avvik_per_sum_AKG,
            'inntektsramme 2026': pd.Series([pd.NA]* 83),
            ...
            'K* lok distribusjonsnett': kostnadsnorm_lokalt_dnett,
            'K* reg distribusjonsnett': kostnadsnorm_regionalt_dnett, 
            'Inntektsramme før kalibrering': inntektsramme_foer_kalibrering,
            'Tillegg i kostnadsnorm for kundevekst': tillegg_i_kostnadsnorm_for_kundevekst,
            'K* etter kalibrering': k_etter_kalibrering
            #'Inntektsramme etter kalibrering' : pd.Series([pd.NA] * 83)
            }
        df: pd.DataFrame = pd.DataFrame(data)

        # --- DEBUG: formula component breakdown ---
        rho = self.config['forutsetninger']['rho']
        akg_term = df['AKG (inkl 1 % arbeids-kapital)'] * (sum_IR_k_per_sum_AKG + sum_renter_paa_avvik_per_sum_AKG) / rho
        debug = pd.DataFrame({
            'ID'                        : df['ID'],
            'S: K* lok dnett'           : df['K* lok distribusjonsnett'],
            'T: K* reg dnett'           : df['K* reg distribusjonsnett'],
            'U: K* transm (=0)'         : kostnadsnorm_transmisjonsnett,
            'S+T+U'                     : df['K* lok distribusjonsnett'].fillna(0) + df['K* reg distribusjonsnett'].fillna(0),
            'I: AKG'                    : df['AKG (inkl 1 % arbeids-kapital)'],
            'N100: sum_IR_k/AKG'        : sum_IR_k_per_sum_AKG,
            'N93: sum_renter/AKG'       : sum_renter_paa_avvik_per_sum_AKG,
            'N100+N93'                  : sum_IR_k_per_sum_AKG + sum_renter_paa_avvik_per_sum_AKG,
            'rho'                       : rho,
            'I*(N100+N93)/rho'          : akg_term,
            'W: Tillegg kundevekst'     : df['Tillegg i kostnadsnorm for kundevekst'],
            'K* etter kalibrering'      : df['K* etter kalibrering'],
        })
        with pd.option_context('display.float_format', '{:.2f}'.format, 'display.max_columns', None, 'display.width', 300):
            print("\n=== DEBUG: K* etter kalibrering komponenter (ID=7) ===")
            print(debug[debug['ID'] == 7].to_string(index=False))
        # --- END DEBUG ---
...
if __name__ == "__main__":
    ...
    #print(deanorm_dnett[['ID'], 'Tillegg i norm'])
    #print(deanorm_dnett)
