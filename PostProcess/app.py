...
# ---------------------------------------------------------------------------
# Pipeline navigation  (query-param driven so <a> cards are truly clickable)
# ---------------------------------------------------------------------------
try:
    active_step = int(st.query_params.get("step", 1))
except (ValueError, TypeError):
    active_step = 1
if active_step not in (1, 2, 3):
    active_step = 1
...
def _card(step_n: int, icon_html: str, label: str) -> str:
    ...
    return f"""
    <a href="?step={step_n}" style="text-decoration:none;">
...
.pipeline-step:hover {{
    background: #254e82;
    box-shadow: 0 4px 14px rgba(46,109,180,0.45);
}}
.pipeline-step-active {{
    background: #2e6db4 !important;
    box-shadow: 0 4px 16px rgba(46,109,180,0.55) !important;
